...
class ParkingCubit extends Cubit<ParkingState> {
  ...
  void selectParking(int parkingId, int userId) {
    try {
      ServiceCall.post(
        {"parking_id": parkingId.toString(), "user_id": parkingId.toString()},
        SVKey.svSelectParking,
        isTokenApi: true,
        withFailure: (response) async {
          emit(ParkingErrorState(response.toString()));
        },
        withSuccess: (response) async {
          if (response.statusCode == HttpStatus.ok) {
            emit(SelectParkingApiResultState());
            emit(ParkingInitial());
          } else {
            emit(ParkingErrorApiResultState(
                ErrorResponse.fromJson(jsonDecode(response.body))));
